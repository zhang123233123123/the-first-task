...
| 4. Baseline 问卷 | http://47.106.117.27:3000/baseline | 6 个 block：Demographics → NFC → IE-4 → AI Use → Writing Exp → CSE |
| 5. 任务 1 简报 | http://47.106.117.27:3000/task/1/brief | 显示任务类型（story/metaphor）和条件提示 |
| 6. 任务 1 工作区 | http://47.106.117.27:3000/task/1/suggestions | 5 分钟计时写作，AI 面板（根据条件不同） |
| 7. 任务 1 后测 | http://47.106.117.27:3000/task/1/survey | 注意力检查 → NASA-TLX → SMI(4 block) → CSE → Ownership |
| 8. 任务 2 简报 | http://47.106.117.27:3000/task/2/brief | 第二个任务的说明 |
| 9. 任务 2 工作区 | http://47.106.117.27:3000/task/2/suggestions | 同任务 1，不同的题目 |
| 10. 任务 2 后测 | http://47.106.117.27:3000/task/2/survey | 同任务 1 后测 |
| 11. 完成页 | http://47.106.117.27:3000/complete | 感谢 + 研究 debrief，5 秒后自动清除 session |
...
简化版流程，无 baseline 量表（只有 demographics），无 post-task 问卷，最后有 2 题 manipulation check。

| 步骤 | 链接 | 说明 |
|------|------|------|
| 1. 入口 | http://47.106.117.27:3000/pilot | Pilot 知情同意 + 4 题 demographics |
| 2. 任务 1 简报 | http://47.106.117.27:3000/task/1/brief | 同正式实验 |
| 3. 任务 1 工作区 | http://47.106.117.27:3000/task/1/suggestions | 同正式实验（但只有 3 个条件） |
| 4. 任务 2 简报 | http://47.106.117.27:3000/task/2/brief | 同正式实验 |
| 5. 任务 2 工作区 | http://47.106.117.27:3000/task/2/suggestions | 同正式实验 |
| 6. Manipulation Check | http://47.106.117.27:3000/pilot/check | 2 题 Likert（验证 friction/provocateur 感知） |
| 7. 完成页 | http://47.106.117.27:3000/pilot/complete | Pilot 感谢页 |
...
| 链接 | 说明 |
|------|------|
| http://47.106.117.27:3000/debug/conditions | **正式实验条件启动器** — 选择 6 个条件之一，跳过 consent/baseline，直接进入任务 |
| http://47.106.117.27:3000/debug/pilot | **Pilot 条件启动器** — 选择 3 个 pilot 条件之一，以 pilot 模式直接进入任务 |
| http://47.106.117.27:3000/debug/data | **数据查看面板** — 浏览所有参与者数据，支持按 Mode（Main/Pilot）、Condition、Completion 筛选，可展开查看 baseline JSON、任务作品、交互日志、后测问卷等原始数据 |
...
| 方法 | 路径 | 说明 |
|------|------|------|
| POST | `/participants/init` | 创建参与者。参数：`condition`（可选，调试用）、`study_mode`（"main"/"pilot"） |
...
| POST | `/responses/log-event` | 追加交互日志事件 |
...
- Debug Pilot（跳过 demographics）：http://47.106.117.27:3000/debug/pilot

### 正式实验测试

- 正式入口：http://47.106.117.27:3000/
- Debug 指定条件（跳过 baseline）：http://47.106.117.27:3000/debug/conditions
- 指定条件直接进入（如 friction）：http://47.106.117.27:3000/consent?condition=friction

### 数据查看

- 全部数据：http://47.106.117.27:3000/debug/data
